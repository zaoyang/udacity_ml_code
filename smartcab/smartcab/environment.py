...
class Environment(object):
    ...
    def reset(self, testing=False, total_trials = 1):
    ...
    def step(self):
        ...
        if(self.verbose == True): # Debugging
            print "Total # of trials: ", self.total_trials
    ...
    def act(self, agent, action):
        ...
        if violation == 0:
            ...
            if action == agent.get_next_waypoint(): # Was it the correct action?
                ...
                self.lastViolation = "Correct move to waypoint"
                reward += 8
            elif action == None and light != 'green': # Was the agent stuck at a red light?
                self.lastViolation = "Safe but not to waypoint"
                # reward += 2 - penalty # (2, 1)
                reward += -1 # (2, 1)
            else: # Valid but incorrect
                # reward += 1 - penalty # (1, 0)
                reward += -1
                self.lastViolation = "Safe but not correct decision and light not red"
        ...
        # Agent attempted invalid move
            ...
            if violation == 1: # Minor violation
                self.lastViolation = "Minor violation"
                reward += -8
            elif violation == 2: # Major violation
                self.lastViolation = "Major violation"
                reward += -20
            elif violation == 3: # Minor accident
                self.lastViolation = "Minor accident"
                reward += -40
            elif violation == 4: # Major accident
                self.lastViolation = "Major accident"
                reward += -50
